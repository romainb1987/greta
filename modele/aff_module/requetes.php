...
function insereData($idformation,$module){
        //insertion du module en cours avec la formation en cours dans la table associative contenu formation
...
function getModules($idTheme){
    ...
    $bdd = getBdd();
    $req = $bdd->prepare('SELECT idModules,Nom_mod FROM modules WHERE idTheme = ? AND  mod_ref = 1');
