    ...
    function chargeListe(){
        ...
        chargeDetail($idTheme);
        
    }
    function chargeDetail($idTheme){

        echo '<label >Selectionnez les Détails desirés</label></br></br>';
        //recup des modules pour le premier theme initilisé
        $listedetails = getDetail($idTheme);
        $deb = true;
        $i = 1;
        echo '<table id="details">';
        //tant il y a des éléments à lire
        while ($Detail = $listedetails->fetch()){
            ...
            echo '<td><label>'.$Detail['Nom_detail'].'<input type="checkbox" name="detail" id="'.$Detail['id_Detail'].'"/></label></td>';
            $i++;
